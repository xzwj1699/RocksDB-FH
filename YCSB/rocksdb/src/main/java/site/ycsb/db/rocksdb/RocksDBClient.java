...
public class RocksDBClient extends DB {
  ...
  private RocksDB initRocksDBWithOptionsFile() throws IOException, RocksDBException {
    ...
    RocksDB.loadLibrary();
    ConfigOptions myConfigOption = new ConfigOptions();
    // myConfigOption.setEnv(Env);
    // OptionsUtil.loadOptionsFromFile(optionsFile.toAbsolutePath().toString(), 
    // Env.getDefault(), options, cfDescriptors);
    OptionsUtil.loadOptionsFromFile(myConfigOption, optionsFile.toAbsolutePath().toString(), options, cfDescriptors);
    dbOptions = options;
  ...
  private RocksDB initRocksDB() throws IOException, RocksDBException {
    ...
    for(final String cfName : cfNames) {
      ...
      if (cfName.equals("default")) {
        ...
        Cache cache = new FHLRUCache(512 * 1024 * 1024, 4);
    ...
    if(cfDescriptors.isEmpty()) {
      ...
      final BlockBasedTableConfig tableOptions = new BlockBasedTableConfig();
      Cache cache = new FHLRUCache(4 * 1024 * 1024 * 1024, 4);
      tableOptions.setBlockCache(cache);
      options.setTableFormatConfig(tableOptions);
      dbOptions = options;
      System.out.println("Init RocksDB with cfDes empty");
      ...
      dbOptions = options;
      System.out.println("Init RocksDB with cfDes");
      final List<ColumnFamilyHandle> cfHandles = new ArrayList<>();
      final RocksDB db = RocksDB.open(options, rocksDbDir.toAbsolutePath().toString(), cfDescriptors, cfHandles);
      for(int i = 0; i < cfNames.size(); i++) {
        System.out.println(i);
        System.out.println(cfNames.get(i));
