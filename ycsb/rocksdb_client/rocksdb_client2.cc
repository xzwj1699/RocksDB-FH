...
void RocksDBClient2::Load(){
	...
	std::cout << "Start load data, target records: " << load_num_ << ", work thread num: "
			  << loader_threads_ << ", per thread load num: " << num << std::endl;
	std::vector<std::thread> threads;
	std::function< void(int, int)> fn;
	// if(options_.enable_spdklogging){
	// 	fn = std::bind(&RocksDBClient2::LogsDBLoader, this, 
	// 					std::placeholders::_1, std::placeholders::_2);
	// }else{
		fn = std::bind(&RocksDBClient2::RocksdDBLoader, this, 
						std::placeholders::_1, std::placeholders::_2);
	// }
	auto start = TIME_NOW;
...
void RocksDBClient2::Work(){
	Reset();
	assert(request_time_ == nullptr);
	assert(read_time_ == nullptr);
	assert(update_time_ == nullptr);
	assert(total_request_time_ == nullptr);
	...
	total_request_time_ = new TimeRecord(request_num_ + 1);
...
#if defined(MONITOR) || defined(RECENCY) || defined(USE_BLOCK)
	std::vector<std::thread> threads_monitor;
	std::function< void(int)> fn_monitor;
	printf("client2 have monitor!\n");
#endif
	// if(options_.enable_spdklogging){
	// 	fn = std::bind(&RocksDBClient2::LogsDBWorker, this, 
	// 					std::placeholders::_1, std::placeholders::_2,
	// 					std::placeholders::_3, std::placeholders::_4);
	// }else{
		fn = std::bind(&RocksDBClient2::RocksDBWorker, this, 
		   			    std::placeholders::_1, std::placeholders::_2,
		   			    std::placeholders::_3, std::placeholders::_4);
#if defined(MONITOR) || defined(RECENCY) || defined(USE_BLOCK)
		fn_monitor = std::bind(&RocksDBClient2::FastHashMonitor, this, std::placeholders::_1);
#endif
	// }
	printf("start time: %s\n", GetDayTime().c_str());
	auto start = TIME_NOW;
...
#if defined(MONITOR) || defined(RECENCY) || defined(USE_BLOCK)
	// if(!options_.enable_spdklogging)
		threads_monitor.emplace_back(fn_monitor, 120); // To optimize
	printf("push monitor!\n");
#endif
	for(auto &t : threads)
		t.join();
#if defined(MONITOR) || defined(RECENCY)
	if(!options_.enable_spdklogging){
		options_.lo_env->monitor_stop();
		for(auto &t : threads_monitor)
			t.join();
	}
#elif defined(USE_BLOCK)
	// if(!options_.enable_spdklogging){
		RocksDBClient2::FH_Set_Status(false);
		for(auto &t : threads_monitor)
			t.join();
	// }
#endif
	...
	printf("Work IOPS: %.3lf K (end to end latency: %.3lf us)\n", request_num_/time*1000*1000/1000, time/request_num_*worker_threads_);
	...
	printf("Block read time: %.3lf us (count: %ld)\n", block_read_time_/read_time_->Size(), read_time_->Size());
	printf("Block avg read time: %.3lf us (count: %ld)\n", block_read_time_/block_read_count_, block_read_count_);
	...
	// For Test
	uint64_t block_add_failures = options_.statistics->getTickerCount(rocksdb::BLOCK_CACHE_ADD_FAILURES);
	uint64_t block_cache_index_miss = options_.statistics->getTickerCount(rocksdb::BLOCK_CACHE_INDEX_MISS);
	uint64_t block_cache_index_hit = options_.statistics->getTickerCount(rocksdb::BLOCK_CACHE_INDEX_HIT);
	uint64_t block_cache_filter_miss = options_.statistics->getTickerCount(rocksdb::BLOCK_CACHE_FILTER_MISS);
	uint64_t block_cache_filter_hit = options_.statistics->getTickerCount(rocksdb::BLOCK_CACHE_FILTER_HIT);
	uint64_t block_cache_data_miss = options_.statistics->getTickerCount(rocksdb::BLOCK_CACHE_DATA_MISS);
	uint64_t block_cache_data_hit = options_.statistics->getTickerCount(rocksdb::BLOCK_CACHE_DATA_HIT);
	uint64_t block_cache_bytes_read = options_.statistics->getTickerCount(rocksdb::BLOCK_CACHE_BYTES_READ);
	uint64_t block_cache_bytes_write = options_.statistics->getTickerCount(rocksdb::BLOCK_CACHE_BYTES_WRITE);
	printf("block cache hit ratio: %.3lf (hit: %ld, miss: %ld)\n", 
			block_hit*1.0/(block_hit+block_miss), block_hit, block_miss);
	// For Test
	printf("block cache add failures: %ld\n", block_add_failures);
	printf("block cache index hit ratio: %.3lf (hit: %ld, miss: %ld)\n",
		   block_cache_index_hit*1.0/(block_cache_index_hit+block_cache_index_miss), \
		   block_cache_index_hit, block_cache_index_miss);
	printf("block cache filter hit ratio: %.3lf (hit: %ld, miss: %ld)\n",
		   block_cache_filter_hit*1.0/(block_cache_filter_hit+block_cache_filter_miss), \
		   block_cache_filter_hit, block_cache_filter_miss);
	printf("block cache data hit ratio: %.3lf (hit: %ld, miss: %ld)\n",
		   block_cache_data_hit*1.0/(block_cache_data_hit+block_cache_data_miss), \
		   block_cache_data_hit, block_cache_data_miss);
	printf("block cache bytes read: %ld, write: %ld\n", \
...
void RocksDBClient2::Warmup(){
	...
	if(workload_wrapper_ == nullptr){
		...
		workload_wrapper_ = new WorkloadWrapper(workload_proxy_, (uint64_t)(request_num_+warmup_num_) + 1, false);
	}
	std::cout << "Start experiment warmup stage" << std::endl;

	auto start = TIME_NOW;
	//const uint64_t warmup_num = floor(request_num_ * warmup_rate_);
	//const uint64_t num = warmup_num / worker_threads_;
	const uint64_t num = warmup_num_ / worker_threads_;
	//printf("Warmup Rate: %.2lf\n", warmup_rate_);
	...
	assert(update_time_ == nullptr);
	assert(total_request_time_ == nullptr);
	request_time_ = new TimeRecord(warmup_num_ + 1);
	read_time_ = new TimeRecord(warmup_num_ + 1);
	update_time_ = new TimeRecord(warmup_num_ + 1);
	total_request_time_ = new TimeRecord(warmup_num_ + 1);
...
// 	}
// #endif
	std::cout << "Finish experiment warmup stage" << std::endl;
	workload_wrapper_->SetCursor(warmup_num_);
	double time = TIME_DURATION(start, TIME_NOW);
	...
	printf("Work latency: %.3lf us\n", request_time_->Sum()/request_time_->Size());
	//printf("Work IOPS: %.3lf K\n", warmup_num_/time*1000*1000/1000);
	printf("Work IOPS: %.3lf K (end to end latency: %.3lf us)\n", warmup_num_/time*1000*1000/1000, time/warmup_num_*worker_threads_);
	printf("Work median latency: %.3lf us\n", request_time_->Tail(0.5));
	printf("Work P999: %.3lfus, P99: %.3lfus, P95: %.3lfus, P90: %.3lfus, P75: %.3lfus\n",
			request_time_->Tail(0.999), request_time_->Tail(0.99), request_time_->Tail(0.95),
		    request_time_->Tail(0.90), request_time_->Tail(0.75));
	...
	delete read_time_;
	delete update_time_;
	delete total_request_time_;
	request_time_ = nullptr;
	read_time_ = nullptr;
	update_time_ = nullptr;
	total_request_time_ = nullptr;
}

void RocksDBClient2::FastHashMonitor(int coreid) {
	SetAffinity(coreid);
	//printf("test1\n");
#ifdef USE_BLOCK
	//printf("%s\n", options_.table_factory->Name());
	//printf("%d\n", reinterpret_cast<rocksdb::BlockBasedTableOptions*>(options_.table_factory->GetOptions())->index_type);
	...
	// block_cache->FH_Scheduler();
#else
	//options_.lo_env->FastHashMonitor();
#endif
}

#ifdef USE_BLOCK
void RocksDBClient2::FH_Set_Status(bool status) {
	//printf("test4\n");
	// std::shared_ptr<rocksdb::Cache> block_cache = reinterpret_cast<rocksdb::BlockBasedTableOptions*>(options_.table_factory->GetOptions())->block_cache;
	// block_cache->FH_Set_Status(status);
}
...
void RocksDBClient2::Set_Record_Reset_Flag(int coreid, bool status){
	// std::shared_ptr<rocksdb::Cache> block_cache = reinterpret_cast<rocksdb::BlockBasedTableOptions*>(options_.table_factory->GetOptions())->block_cache;
	// block_cache->Set_Record_Reset_Flag(coreid, status);
}
#endif

void RocksDBClient2::RocksDBWorker(uint64_t num, int coreid, bool is_warmup, bool is_master){
	SetAffinity(coreid);
...
#ifdef USE_BLOCK
		if(i % Granularity == 0)
			total_request_time_->Insert(time);
#endif
		if(i % 2 == 0){
	...
	if(is_warmup) {
		...
		block_read_count_ += rocksdb::get_perf_context()->block_read_count;
		mutex_.unlock();
		return;
	}
...
void RocksDBClient2::LogsDBWorker(uint64_t num, int coreid, bool is_warmup, bool is_master){
	// SetAffinity(coreid);
	// rocksdb::SetPerfLevel(rocksdb::PerfLevel::kEnableTimeExceptForMutex);
	// rocksdb::get_perf_context()->Reset();
	// rocksdb::get_iostats_context()->Reset();
...
void RocksDBClient2::LogsDBLoader(uint64_t num, int coreid){
	/*SetAffinity(coreid);
	rocksdb::SetPerfLevel(rocksdb::PerfLevel::kEnableTimeExceptForMutex);
	rocksdb::get_perf_context()->Reset();
	rocksdb::get_iostats_context()->Reset();
...
void RocksDBClient2::RocksdDBLoader(uint64_t num, int coreid){
	rocksdb::SetPerfLevel(rocksdb::PerfLevel::kEnableTimeExceptForMutex);
	rocksdb::get_perf_context()->Reset();
	rocksdb::get_iostats_context()->Reset();
