...
int main(const int argc, const char *argv[]){
	...
	if(dbname == "logsdb_ll0" && !options.auto_config){
		...
		env->SetBgThreadCores(1, rocksdb::Env::LOW);
	}
	...
	options.ssdlogging_path = "trtype:PCIe traddr:0000:25:00.0";

	// system(("rm " + data_dir + "/*").c_str());
	// system(("rm " + log_dir + "/*").c_str());
	// printf("loading database....\n");
	// if(is_load == 0)
	// 	system(("cp /home/spdk/nvme_md/rocksdb_bak_512gb/* " + data_dir + "/").c_str());
	...
	// Run
	{
		...
		printf("------------------------------------------\n");
		fflush(stdout);
		ycsbc::RocksDBClient5 rocksdb_client5(&wp, options, write_options, read_options, data_dir, loader_threads,
					  load_num, worker_threads, requests_num, async_num);
		if(is_load == 1){
			rocksdb_client5.Load();
		}else{
			//rocksdb_client2.Warmup();
			rocksdb_client5.Work();
		}
...
void ParseCommandLine(int argc, const char *argv[], utils::Properties &props) {
	if(argc != 9){
		printf("usage: <workload file> <threads num> <data_dir> <log_dir> \
					   <enable_ssdlogging> <max_level> <is_load> <dbname>\n");
		exit(0);
	}
	//1. workload file
...
void Move(std::string src, std::string des, const int max_level){
	...
	//1. get file
	FILE *inputfile = fopen((src + "/spdk.sst").c_str(), "r");
	...
	for(int i=0; i<MAXLEVEL; i++){
    	...
    	for(auto &file : files_level[i]){
    		std::string src_fname = src + "/" + file;
    		std::string des_fname = des + "/" + file;
    		system(("mv " + src_fname + " " + des_fname).c_str());
		}
	}
	fflush(stdout);
}