...
class RocksDBClient2{
	private:
		WorkloadProxy *workload_proxy_;
		ROCKSDB_NAMESPACE::DB* db_;
		const ROCKSDB_NAMESPACE::Options options_;
		const ROCKSDB_NAMESPACE::WriteOptions write_options_;
		const ROCKSDB_NAMESPACE::ReadOptions read_options_;
		...
		struct TimeRecord{
		  ...
		  public:
			...
			void Join(TimeRecord* time){
				...
				//printf("Join size: %ld, capacity: %ld\n", size_, capacity_);
				assert(size_ <= capacity_);
...
#ifdef USE_BLOCK
		TimeRecord *total_request_time_ = nullptr;
		// TimeRecord *req_time[16];
	...
	public:
		RocksDBClient2(WorkloadProxy *workload_proxy, rocksdb::Options options, rocksdb::WriteOptions write_options,
					  rocksdb::ReadOptions read_options, std::string data_dir, int loader_threads,
					  uint64_t load_num, int worker_threads, uint64_t warmup_num, uint64_t request_num, int async_num):
		...
		void ProduceSPDKSST(){
			...
			FILE *output = fopen("spdk.sst", "w");
			for(auto &meta : metadata){
				fprintf(output, "level is %d , fname is %s\n",meta.level, meta.name.substr(1).c_str());
			}
			fclose(output);
		}
	...
	private:
		...
		bool FH_Get_Status();
		void Set_Record_Reset_Flag(int coreid, bool status);
		bool Get_Record_Reset_Flag(int coreid);
	#endif
