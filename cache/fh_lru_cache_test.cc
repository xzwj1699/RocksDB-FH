...
class FHLRUCacheTest : public testing::Test {
 public:
  FHLRUCacheTest() {}
  ~FHLRUCacheTest() override { DeleteCache(); }

  void DeleteCache() {
    if (cache_ != nullptr) {
      cache_->~FHLRUCacheShard();
  ...
  void NewCache(size_t capacity, double high_pri_pool_ratio = 0.0,
    ...
    DeleteCache();
    cache_ = reinterpret_cast<FHLRUCacheShard*>(
        port::cacheline_aligned_alloc(sizeof(FHLRUCacheShard)));
    new (cache_) FHLRUCacheShard(capacity, /*strict_capacity_limit=*/false,
 ...
 private:
  FHLRUCacheShard* cache_ = nullptr;
  Cache::EvictionCallback eviction_callback_;
};

TEST_F(FHLRUCacheTest, BasicLRU) {
  NewCache(5);
...
TEST_F(FHLRUCacheTest, LowPriorityMidpointInsertion) {
...
TEST_F(FHLRUCacheTest, BottomPriorityMidpointInsertion) {
...
TEST_F(FHLRUCacheTest, EntriesWithPriority) {
  // Allocate 2 cache entries to high-pri pool and 2 to low-pri pool.
  NewCache(6, /* high_pri_pool_ratio */ 0.35, /* low_pri_pool_ratio */ 0.35);
