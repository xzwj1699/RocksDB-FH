...
namespace my_random {
  ...
  public:
    ...
    explicit ConcurrentRandom(uint32_t s) : seed_(GoodSeed(s)) {}
    ...
    // additional locking
    static ConcurrentRandom* GetTLSInstance() {
      static __thread ConcurrentRandom* tls_instance;
      static __thread std::aligned_storage<sizeof(ConcurrentRandom)>::type tls_instance_bytes;
      auto rv = tls_instance;
      if (UNLIKELY(rv == nullptr)) {
        size_t seed = std::hash<std::thread::id>()(std::this_thread::get_id());
        rv = new (&tls_instance_bytes) ConcurrentRandom((uint32_t)seed);
        tls_instance = rv;
      }
      return rv;
    }
...
struct FHLRUHandle : public Cache::Handle {
